...
bool Network::activate(  std::vector< double >& outActivations ) 
{
	list<NNode*>::iterator curnode;
	list<Link*>::iterator curlink;
	...
	while(outputsoff()||!onetime) {
		...
		for(curnode=all_nodes.begin();curnode!=all_nodes.end();++curnode) {
			...
			outActivations.push_back( (*curnode)->activation );
		...
		for(curnode=all_nodes.begin();curnode!=all_nodes.end();++curnode) 
		{
			...
			//outActivations.push_back( (*curnode)->activesum );
			outActivations.push_back( (*curnode)->activation );  // is this correct for sensors??
