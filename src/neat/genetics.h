...
public:
	...
	void print_to_stream(ostream &outFile); //Print gene to a stream- called from Genome
...
public:  
    /// Ensures that newnodes contains all the input and output nodes
    /// that this (the assumed parent genome) does.
    void ensureChildIONodes( list<NNode*>& newnodes );
    
    /// Verify the number of output nodes is correct.
    void verifyOutputCount( size_t expected = 5 );
    
    /// Verify the number of output nodes in the testNodes list is correct.
    static void verifyOutputCount( const list<NNode*>& testNodes, size_t expected = 5)
    {
#ifdef DEBUG
        size_t count = 0;
        list<NNode*>::const_iterator iter; 
        for( iter = testNodes.begin(); iter != testNodes.end(); ++iter ) 
        {
            NNode* node = *iter;
        ...
        if( count != expected )
        {
            ...
            std::cerr << "Output Count Error:  " << count << " != " << expected << "\n";
            return;
        }
#endif
    }
    ...
    //new Genome(count,   i,     o, randint(0,nmax),nmax,r,linkprob)
	Genome(int new_id,int i, int o, int n,int nmax, bool r, double linkprob) {
		...
		for(ncount=1;ncount<=i;ncount++) {
			if (ncount<i)
				newnode=new NNode(SENSOR,ncount,INPUT_NODE);
    ...
    void print_to_stream(ostream &outFile); //Dump this genome to specified stream

	void print_to_file(ofstream &outFile); //Dump this genome to specified file

	void print_to_filename(const char *filename); //Dump genome to file
...
public:
	...
	Population(int size,int i,int o, int nmax, bool r, double linkprob) {
		...
		for(count=0;count<size;count++) {
            ...
            new_genome=new Genome(count,i,o, randint(0,max_initial_node_count),nmax,r,linkprob);
