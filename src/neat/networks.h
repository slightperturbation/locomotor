...
class Trait : public NEAT {
 ...
 public:
  ...
  ~Trait() {
    ...
    void print_to_stream(ostream &outFile);  //Dump trait to a stream
...
class NNode : public NEAT {
 ...
 public:
  ...
  void flushback_check(list<NNode*> &seenlist);  //Verify flushing for debuginh
    ...
    void  print_to_stream(ostream &outFile); //Print the node to a stream
...
class Network : public NEAT {
 ...
 public:
  ...
  list<NNode*> getAllNodes( void ) { return all_nodes; }
  ...
  bool activate( std::vector< double >& outActivations ); //Activates the net such that all outputs are active
