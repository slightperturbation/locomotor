...
ostream& operator<< (ostream& os, const Genome *thegenome) {
    ...
    for(curnode=nodes.begin();curnode!=nodes.end();++curnode) {
        if (((*curnode)->gen_node_label)==INPUT_NODE) cout<<"I";
...
void 
Genome
::verifyOutputCount( size_t expected )
{
    verifyOutputCount( nodes, expected );
}
...
Network *Genome::genesis(int id)
{
    ...
    for( curnode = nodes.begin(); curnode != nodes.end(); ++curnode ) 
    {
        ...
        if (((*curnode)->gen_node_label)==INPUT_NODE) 
    ...
    //if( outlist.size() != 5 )
    //{
    //    std::cerr << "in genome::genesis(id), just past node creation, but # of outnodes is != 5.  (" << outlist.size() << ")\n";
    //}
    /// DEBUG--- DEX
...
//    if( outlist.size() != 5 )
//    {
//        std::cerr << "in genome::genesis(id), just past edge creation, but # of outnodes is != 5.  (" << outlist.size() << ")\n";
//    }
//    /// DEBUG--- DEX
...
void Trait::print_to_stream(ostream &outFile) { 
...
void NNode::print_to_stream(ostream &outFile) {
...
void Gene::print_to_stream(ostream &outFile) {
    outFile<<"gene ";
...
void Genome::print_to_stream(ostream &outFile) {
    vector<Trait*>::iterator curtrait;
    list<NNode*>::iterator curnode;
    list<Gene*>::iterator curgene;
    ...
    for(curtrait=traits.begin();curtrait!=traits.end();++curtrait) {
        ...
        (*curtrait)->print_to_stream(outFile);
    }
    
    //Output the nodes
    for(curnode=nodes.begin();curnode!=nodes.end();++curnode) {
        (*curnode)->print_to_stream(outFile);
    }
    
    //Output the genes
    for(curgene=genes.begin();curgene!=genes.end();++curgene) {
        (*curgene)->print_to_stream(outFile);
    }
...
Genome *Genome::duplicate(int new_id) 
{
    ...
    for(curnode=nodes.begin();curnode!=nodes.end();++curnode) {
        ...
        else 
        {
            ...
            while( *curtrait 
                  && ((*curtrait)->trait_id) != (((*curnode)->nodetrait)->trait_id)
                )
            {
                ++curtrait;
            }
...
void Genome::mutate_link_weights(double power,double rate,mutator mut_type) 
{
    ...
    */
    ...
    // DEXTER - 29 Aug 09
    //endpart=gene_total*0.8;
    endpart = gene_total * (1.0 - NEAT::new_genes_severe_mutate);
...
Genome *Genome::mate_multipoint(Genome *g,int genomeid,double fitness1,double fitness2) 
{
            ...
            if (!skip) {
                ...
                if (inode->node_id<onode->node_id) {
                    ...
                    if (curnode==newnodes.end()) 
                    {
                        ...
                        node_insert(newnodes,new_onode);  //INSERT NODE
                    }
                    else 
                    {
                        new_onode=(*curnode);
                ...
                else 
                {
                    ...
                    if (curnode==newnodes.end()) 
                    {
                        ...
                        node_insert(newnodes,new_inode);  //INSERT NODE
                    }
                    else {
                        new_inode=(*curnode);
    ...
    /// NODES ARE ONLY ADDED IF GENES REFERENCE THEM
    /// SO OUTPUT GENES GET LEFT BEHIND

    verifyOutputCount();
    ensureChildIONodes( newnodes );
    verifyOutputCount( newnodes );
    
    new_genome=new Genome(genomeid,newtraits,newnodes,newgenes);
    
    new_genome->verifyOutputCount();
...
void
Genome
::ensureChildIONodes( list<NNode*>& newnodes )
{
    vector<Trait*> newtraits; 
    int nodetraitnum;  //Trait number for a NNode
    
    list<NNode*>::const_iterator iter; 
    for( iter = nodes.begin(); iter != nodes.end(); ++iter ) 
    {
        NNode* node = *iter;
        if( ((node->gen_node_label) == OUTPUT) || ((node->gen_node_label) == INPUT_NODE) )
        { 
            // make sure all of 1st parent's outputs are here
            list<NNode*>::iterator curnode = newnodes.begin();
            while(   ( curnode != newnodes.end() ) 
                  && ( (*curnode)->node_id != node->node_id ) 
                  )
            {
                ++curnode;
            }
            if (curnode==newnodes.end()) 
            {
                ...
                {
                    nodetraitnum = ((node->nodetrait)->trait_id)-(*(traits.begin()))->trait_id;			       
                }
                */
                NNode* newnode = new NNode( node, 0 ) ; //traits[nodetraitnum] );  /* traits taken from this parent */
                node_insert( newnodes, newnode );
            }
        }
    }    
}
...
Genome *Genome::mate_multipoint_avg(Genome *g,int genomeid,double fitness1,double fitness2) 
{
    ...
    Genome* babyGenome = new Genome(genomeid,newtraits,newnodes,newgenes);
    babyGenome->verifyOutputCount();
...
void Organism::update_phenotype() {
    ...
    std::cout << "update_phenotype no longer does anything\n";
...
bool Population::spawn(Genome *g,int size) {
    ...
    for(count=1;count<=size;count++) {
        ...
        new_genome->mutate_link_weights(1.0,0.01,GAUSSIAN);
...
bool Population::epoch(int generation) {
    ...
    if (generation>1) {
		...
		{
			//std::cout << "Too few species (num=" << num_species << " < target=" 
			//	<< num_species_target << ") species compat_threshold by " 
			//	<< compat_mod << " to " << (compat_threshold-compat_mod) << "\n";
            compat_threshold-=compat_mod;
		}
        else if (num_species>num_species_target)
		{
			//std::cout << "Too many species (num=" << num_species << " < target=" 
			//	<< num_species_target << ") species compat_threshold by " << compat_mod << " to " << (compat_threshold-compat_mod) << "\n";
			compat_threshold+=compat_mod;
		}
	...
	//  the system parameter babies_stolen (when babies_stolen > 0)
		...
		while ((stolen_babies<NUM_STOLEN)&&
			...
			if ( (curspecies!=sorted_species.end()) && (stolen_babies>=one_tenth_stolen) )
			{
