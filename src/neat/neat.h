...
class NEAT { 
 ...
 public:
  ...
  // Added by Dexter
  static int num_species_target; /* The number of species to aim for.  If too few, reduce threshold, etc. */
  // Added by Dexter
  static double initial_link_prob; /* probability of initial link between starting neurons */
    static int max_initial_node_count; /* between 0 and max_initial_node_count nodes in initial random population */
    static double new_genes_severe_mutate; /* % of newest genes to severely mutate, NEAT-standard uses 0.2 */
  ...
  static int num_runs; /* The number of runs to average over in an experiment */
    ...
    static int harness_generations;
  ...
  static bool load_neat_params(const char *filename) {
    ...
    paramFile>>curword;
    /* ADDED BY DEXTER */
    paramFile>>num_species_target;
    paramFile>>curword;
	paramFile>>initial_link_prob;
	paramFile>>curword;
      paramFile>>max_initial_node_count;
      paramFile>>curword;
      paramFile>>new_genes_severe_mutate;
      paramFile>>curword;
	/* END ADDED BY DEXTER */
    ...
    paramFile>>num_runs;
      paramFile>>curword;
      paramFile>>harness_generations;
      paramFile>>generation_limit;
    paramFile.close();
    ...
    /*DEX*/
    cout<<"num_species_target="<<num_species_target<<endl;    
	cout<<"initial_link_prob="<<initial_link_prob<<endl;
      cout<<"max_initial_node_count="<<max_initial_node_count<<endl;
      cout<<"new_genes_severe_mutate="<<new_genes_severe_mutate<<endl;
	/*END DEX*/
    ...
    cout<<"num_runs="<<num_runs<<endl;
      cout<<"harness_generations="<<harness_generations<<endl;
  ...
  //    ofstream paramFile(filename,ios::in);
 static void write_neat_params(ofstream& outFile) {
    outFile<<"trait_param_mut_prob="<<trait_param_mut_prob<<endl;
    outFile<<"trait_mutation_power="<<trait_mutation_power<<endl;
    outFile<<"linktrait_mut_sig="<<linktrait_mut_sig<<endl;
    outFile<<"nodetrait_mut_sig="<<nodetrait_mut_sig<<endl;
    outFile<<"weight_mut_power="<<weight_mut_power<<endl;
    outFile<<"recur_prob="<<recur_prob<<endl;
    outFile<<"disjoint_coeff="<<disjoint_coeff<<endl;
    outFile<<"excess_coeff="<<excess_coeff<<endl;
    outFile<<"mutdiff_coeff="<<mutdiff_coeff<<endl;
    outFile<<"compat_threshold="<<compat_threshold<<endl;
    /*DEX*/
    outFile<<"num_species_target="<<num_species_target<<endl;    
	outFile<<"initial_link_prob="<<initial_link_prob<<endl;
     outFile<<"max_initial_node_count="<<max_initial_node_count<<endl;
     outFile<<"new_genes_severe_mutate="<<new_genes_severe_mutate<<endl;
	/*END DEX*/
    outFile<<"age_significance="<<age_significance<<endl;
    outFile<<"survival_thresh="<<survival_thresh<<endl;
    outFile<<"mutate_only_prob="<<mutate_only_prob<<endl;
    outFile<<"mutate_random_trait_prob="<<mutate_random_trait_prob<<endl;
    outFile<<"mutate_link_trait_prob="<<mutate_link_trait_prob<<endl;
    outFile<<"mutate_node_trait_prob="<<mutate_node_trait_prob<<endl;
    outFile<<"mutate_link_weights_prob="<<mutate_link_weights_prob<<endl;
    outFile<<"mutate_toggle_enable_prob="<<mutate_toggle_enable_prob<<endl;
    outFile<<"mutate_gene_reenable_prob="<<mutate_gene_reenable_prob<<endl;
    outFile<<"mutate_add_node_prob="<<mutate_add_node_prob<<endl;
    outFile<<"mutate_add_link_prob="<<mutate_add_link_prob<<endl;
    outFile<<"interspecies_mate_rate="<<interspecies_mate_rate<<endl;
    outFile<<"mate_multipoint_prob="<<mate_multipoint_prob<<endl;
    outFile<<"mate_multipoint_avg_prob="<<mate_multipoint_avg_prob<<endl;
    outFile<<"mate_singlepoint_prob="<<mate_singlepoint_prob<<endl;
    outFile<<"mate_only_prob="<<mate_only_prob<<endl;
    outFile<<"recur_only_prob="<<recur_only_prob<<endl;
    outFile<<"pop_size="<<pop_size<<endl;
    outFile<<"dropoff_age="<<dropoff_age<<endl;
    outFile<<"newlink_tries="<<newlink_tries<<endl;
    outFile<<"print_every="<<print_every<<endl;
    outFile<<"babies_stolen="<<babies_stolen<<endl;
    outFile<<"num_runs="<<num_runs<<endl;
     outFile<<"harness_generations="<<harness_generations<<endl;
     outFile<<"generation_limit="<<generation_limit<<endl;
